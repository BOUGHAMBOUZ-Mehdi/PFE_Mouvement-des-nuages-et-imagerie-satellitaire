...
def chanvese(I, init_mask, fich, max_its=200, alpha=0.2, thresh=0, color='r', display=False):
    I = I.astype(np.float)
    ...
    if display:
        plt.ion()
        show_curve_and_phi(I, phi, color)
    ...
    if display:
        show_curve_and_phi(I, phi, color)
	plt.axis('off')
        plt.savefig('./imgs_chanvese/'+fich, bbox_inches='tight')
    # Make mask from SDF
...
def show_curve_and_phi(I, phi, color):
    plt.cla()
    plt.imshow(I, cmap='gray')
    plt.contour(phi, 0, colors=color)
    plt.draw()
...
if __name__ == "__main__":
	...
	for fichier in fichiers:
		img = nd.imread(path+fichier, flatten=True)	
		mask = np.zeros(img.shape)
	    	#mask2 = np.zeros(img.shape)
	    	mask[10:100, 700:900] = 1
	    	mask[200:500, 350:900] = 1
		print i
	    	chanvese(img, mask, fichier, max_its=2000, display=True, alpha=1.0)
		i+=1
	t1 = time.clock()
	print 'Function elapsed in: ', t0-t1, 's'
	print 'Function elapsed in: ', time.strftime('%H:%M:%S', time.gmtime(t0-t1))
